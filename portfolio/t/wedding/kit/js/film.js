/* OAKHOLLOW — the film engine. Canvas scrub over pre-extracted frames with an
...
(function () {
  'use strict';
  const F = window.OAKHOLLOW_FRAMES;
  ...
  if (reduced || !F.count) {
    ...
    if (JUMP !== null) {
       ...
       * fonts shift layout enough to move which section sits under the header probe. */
  ...
  function updateGauge(pp) {
    if (!gDepth) return;
    let word = C.lightWords[0][1];
    for (const [at, w] of C.lightWords) { if (pp >= at) word = w; }
    gDepth.textContent = word;
    let start = 0;
  ...
  /* ---------- ambient dawn mist over the opening ---------- */
  ...
  function initAmbient() {
    ...
    g.addColorStop(0, 'rgba(235,238,242,.4)');
    g.addColorStop(1, 'rgba(235,238,242,0)');
    sctx.fillStyle = g; sctx.fillRect(0, 0, 64, 64);
    parts = Array.from({ length: 22 }, (_, i) => ({
      x: Math.sin(i * 127.3) * 0.5 + 0.5, y: 0.5 + (Math.sin(i * 91.7) * 0.5 + 0.5) * 0.42,
      z: Math.sin(i * 53.1) * 0.5 + 0.5, ph: i * 1.7
    }));
  }
  function drawAmbient(pp, t) {
    if (!ambCx) return;
    const a = Math.max(0, 1 - pp / 0.08);
    ambCx.clearRect(0, 0, ambient.width, ambient.height);
    if (a === 0) return;
    for (const q of parts) {
      const sz = (90 + q.z * 220) * dpr;
      const drift = ((t * 0.00001 * (0.4 + q.z)) + q.x) % 1.2;
      const bob = Math.sin(t * 0.00045 + q.ph) * 10 * dpr;
      ambCx.globalAlpha = a * (0.04 + q.z * 0.08) * (0.75 + 0.25 * Math.sin(t * 0.0008 + q.ph));
  ...
  function onLoadTick() {
    ...
    if (pctEl) pctEl.textContent = 'Setting the meadow · ' + pct + '%';
