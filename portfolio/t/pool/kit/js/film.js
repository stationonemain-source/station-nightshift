/* GLASS — the film engine. Canvas scrub over pre-extracted frames with an
 * ImageBitmap sliding window (see scroll-film engine notes): every draw is a GPU
 * blit, never a synchronous JPEG decode. Implements the dev contract:
 * ?jump=<y> lands pre-scrolled + settled, window.__ready gates screenshots. */
(function () {
  'use strict';
  const F = window.GLASS_FRAMES;
  ...
  if (reduced || !F.count) {
    ...
    if (JUMP !== null) requestAnimationFrame(() => {
  ...
  /* ---------- frame store + concurrency-capped pump ---------- */
  const COUNT = F.count;
  ...
  let inflight = 0;
  const INFLIGHT = 10;
  /* Load queue, re-sortable around the playhead so deep links fill outward
   * from where the visitor actually landed instead of starving at frame 0. */
  ...
  const B_AHEAD = 12, B_KEEP = 20;
  let bmpCenter = -999;
  function ensureBitmaps(center) {
    /* Keep rescanning until the playhead's own bitmap exists — images stream in
     * after the first pass, and an early return here would strand the fallback frame. */
    ...
    let budget = 5; // stagger decode kickoffs — a 19-wide burst is a visible hitch on weak GPUs
    for (let i = lo; i <= hi; i++) {
      if (bitmaps.has(i) || decoding.has(i) || !images[i]) continue;
      if (--budget < 0) { bmpCenter = -999; break; } // finish the window next tick
      decoding.add(i);
  ...
  let displayed = 0, displayedExact = false;
  ...
  /* ---------- depth gauge ---------- */
  const DEPTH = [[0, 0], [0.28, 0], [0.36, 4.2], [0.5, 8.5], [0.62, 8.0], [0.8, 2.0], [0.88, 0], [1, 0]];
  function depthAt(pp) {
    for (let i = 1; i < DEPTH.length; i++) {
      if (pp <= DEPTH[i][0]) {
        const [x0, y0] = DEPTH[i - 1], [x1, y1] = DEPTH[i];
  ...
  function updateGauge(pp) {
    if (!gDepth) return;
    gDepth.textContent = depthAt(pp).toFixed(1) + ' ft';
    let start = 0;
  ...
  /* ---------- ambient steam over the opening ---------- */
  ...
  function initAmbient() {
    ...
    const g = sprite.getContext('2d').createRadialGradient(32, 32, 2, 32, 32, 30);
    g.addColorStop(0, 'rgba(220,235,240,.55)');
    g.addColorStop(1, 'rgba(220,235,240,0)');
    const sctx = sprite.getContext('2d');
    sctx.fillStyle = g; sctx.fillRect(0, 0, 64, 64);
    parts = Array.from({ length: 26 }, (_, i) => ({
      x: Math.sin(i * 127.3) * 0.5 + 0.5, y: 0.42 + (Math.sin(i * 91.7) * 0.5 + 0.5) * 0.5,
  ...
  function drawAmbient(pp, t) {
    ...
    for (const q of parts) {
      const sz = (60 + q.z * 160) * dpr;
      const drift = ((t * 0.000016 * (0.4 + q.z)) + q.x) % 1.15;
      const bob = Math.sin(t * 0.0006 + q.ph) * 12 * dpr;
      ambCx.globalAlpha = a * (0.05 + q.z * 0.1) * (0.7 + 0.3 * Math.sin(t * 0.0009 + q.ph));
      ambCx.drawImage(sprite, drift * ambient.width - sz / 2, q.y * ambient.height + bob - sz / 2, sz, sz);
    }
    ambCx.globalAlpha = 1;
  }
  ...
  function onLoadTick() {
    ...
    if (pctEl) pctEl.textContent = 'Filling · ' + pct + '%';
