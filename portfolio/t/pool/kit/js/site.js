/* GLASS — shared renderer. Reads window.GLASS_CONTENT, renders header, after-film
 * sections and footer on both tiers. Premium-only film logic lives in film.js. */
(function () {
  'use strict';
  const C = window.GLASS_CONTENT;
  const tier = document.body.dataset.tier || 'premium';
  const root = tier === 'simple' ? '../' : '';

  const MARK_SVG =
    '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="2.6" y="2.6" width="18.8" height="18.8" rx="4.5" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M5.6 12.6c2.1-2.1 4.2 2.1 6.4 0s4.3 2.1 6.4 0" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>';
    ...
    const gauge =
      tier === 'premium'
        ? '<div class="gauge" aria-hidden="true">' +
          '<span class="g-label">Depth</span><span class="g-depth" id="gDepth">0.0 ft</span>' +
          '<span class="g-bar"><i class="g-fill" id="gFill"></i></span>' +
          '<span class="g-ch" id="gCh">01 · The Surface</span></div>'
        : '';
  ...
  function build() {
    sec.push(
      // <section>, not <div>: this is the about landmark, and a div is invisible to
      // every structural/outline reader (ours included).
      '<section class="landing" id="about" data-hdr="dark"><div class="landing-inner">' +
        // Stays a <p>: the page's real h1 is the film's opening beat title, which
        // prerender now bakes into the shell. Two h1s is worse than none.
        '<p class="lede display rv">' + C.manifesto.lede.replace('in water', '<span class="accent">in water</span>') + '</p>' +
        // a plain definitional heading -- the one thing an AI crawler can quote back
        // when someone asks "what does <business> do?"
        '<h2 class="kicker rv rv-d1">What ' + C.brand.name + ' does</h2>' +
        '<p class="body rv rv-d1">' + C.manifesto.body + '</p>' +
      '</div>' +
      // The stats band is opt-in. It used to ship hard-coded "300+ pools designed &
      // built / 16 yrs / 25 yr structural warranty" -- claims about the CLIENT's
      // business that nobody had verified, on every demo built from this kit. An
      // empty C.stats now drops the band entirely; fill it from what the client
      // actually tells us and it comes back.
    ...
    sec.push(
      '<section class="section" id="builds" data-hdr="light" aria-labelledby="builds-h">' +
        '<div class="section-head"><p class="kicker rv">' + C.builds.kicker + '</p>' +
        '<h2 id="builds-h" class="display rv rv-d1">' + C.builds.title + '</h2></div>' +
        C.builds.items.map((b) =>
          '<article class="build rv">' +
            '<div class="build-media"><span class="tag">' + b.place + '</span>' +
            '<img src="' + root + b.img + '" alt="' + b.name + ' — ' + b.spec + '" loading="lazy" width="1248" height="936"></div>' +
            '<div class="build-copy"><h3>' + b.name + '</h3>' +
    ...
    sec.push(
      '<section class="section on-dark" id="process" data-hdr="dark" aria-labelledby="process-h">' +
        '<div class="section-head"><p class="kicker rv">' + C.process.kicker + '</p>' +
        '<h2 id="process-h" class="display rv rv-d1">' + C.process.title + '</h2></div>' +
        '<div class="process-grid">' +
          '<figure class="process-media rv"><img src="' + root + C.process.detailImg + '" alt="' +
            C.process.detailCaption + '" loading="lazy" width="1248" height="936">' +
            '<figcaption>' + C.process.detailCaption + '</figcaption></figure>' +
          '<ol class="steps">' +
    ...
    sec.push(
      '<section class="section" id="services" data-hdr="light" aria-labelledby="services-h">' +
        '<div class="section-head"><p class="kicker rv">' + C.services.kicker + '</p>' +
        '<h2 id="services-h" class="display rv rv-d1">' + C.services.title + '</h2></div>' +
    ...
    sec.push(
      '<section class="financing" id="financing" data-hdr="dark" aria-labelledby="financing-h">' +
        '<div class="financing-inner"><div>' +
          '<p class="kicker rv" style="color:var(--amber)">' + C.financing.kicker + '</p>' +
    ...
    sec.push(
      '<section class="section" data-hdr="light" aria-label="Client words">' +
        '<div class="section-head"><p class="kicker rv">Word of mouth</p>' +
        '<h2 class="display rv rv-d1">Heard poolside.</h2></div>' +
        '<div class="quotes">' +
    ...
    sec.push(
      '<section class="section contact" id="contact" data-hdr="dark" aria-labelledby="contact-h">' +
        '<div class="contact-grid">' +
          '<div class="contact-copy"><p class="kicker rv">' + C.contact.kicker + '</p>' +
          '<h2 id="contact-h" class="display rv rv-d1" style="font-size:var(--text-h2)">' + C.contact.title + '</h2>' +
          ...
          '<ul class="areas rv rv-d3" aria-label="Service area">' +
          ...
          '<form class="lead rv rv-d1" novalidate>' +
            ...
            '<div class="field-row">' +
              ...
              // the date every one of these shops asks for on the phone anyway
              '<div class="field"><label for="f-date">Preferred date</label><input id="f-date" name="date" type="date"></div>' +
            '</div>' +
            '<div class="field"><label for="f-type">Project type</label><select id="f-type" name="type">' +
              C.contact.projectTypes.map((p) => '<option>' + p + '</option>').join('') + '</select></div>' +
            '<div class="field"><label for="f-note">The backyard, in a sentence</label><textarea id="f-note" name="note" rows="3"></textarea></div>' +
            ...
            '<p class="form-note">No spam, no pressure.' +
              (C.brand.license ? ' ' + C.brand.license + '.' : '') + '</p>' +
          '</form>' +
          '</div>' +
        '</div>' +
      '</section>'
    ...
    document.getElementById('siteFoot').innerHTML =
      '<div class="foot">' +
        ...
        // ', Texas' used to be hard-coded here -- wrong for every client outside TX.
        // The state now rides on C.brand.region (set by the geo rewrite) or is omitted.
        '<span>' + C.brand.city + (C.brand.region ? ', ' + C.brand.region : '') +
          (C.brand.founded ? ' · Since ' + C.brand.founded : '') + '</span></div>' +
        ...
        '</div>' +
        '<p class="foot-legal">&copy; 2026 ' + C.brand.legal +
          (C.brand.license ? ' · ' + C.brand.license : '') + '</p>' +
      '</div>';
  }
  ...
  /* ---------- header theme: DOM probe owns it once the film/hero has scrolled by ----------
   * The film's canvas-luminance sampler owns the header while the film is under it
   * (film.js gates itself the same way); past that, the section under the header decides. */
  ...
  /* Dev contract: after a ?jump lands, anything at or above the viewport must be
   * settled — a programmatic jump can skip right past the IntersectionObserver. */
    ...
    layer.innerHTML = C.beats
      .map((b) => {
        ...
        const ctas = (b.ctas || [])
          ...
          .join('');
        return (
          '<div class="beat" data-id="' + b.id + '"><div class="beat-inner">' +
